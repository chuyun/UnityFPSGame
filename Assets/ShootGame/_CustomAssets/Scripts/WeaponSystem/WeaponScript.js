...
public class WeaponScript extends MonoBehaviour {
	...
	function Start (){
		...
		//Camera recoil
		camDefaultRotation = GetComponent.<Camera>().main.transform.localRotation;
		
		//Aim setup
		defaultFov = GetComponent.<Camera>().main.fieldOfView;
	...
	function machineGunOneShot () {
			...
			GetComponent.<AudioSource>().clip = machineGun.fireSound;
	...
	function machineGunReload () {
		...
		//Play reload sound
		GetComponent.<AudioSource>().clip = machineGun.reloadSound;
		GetComponent.<AudioSource>().Play();
	...
	function grenadeLauncherOneShot (){
		...
		GetComponent.<AudioSource>().clip = grenadeLauncher.fireSound;
		GetComponent.<AudioSource>().Play();
		if(grenadeLauncher.shotDelay == 0){
			//Send message to weapon animation script
			if(aimed && Aim.playAnimation){
	...
	function grenadeLauncherReload(){
		...
		//Play reload sound
		GetComponent.<AudioSource>().clip = grenadeLauncher.reloadSound;
		GetComponent.<AudioSource>().Play();
	...
	function shotGunOneShot () {
		...
		for (var i : int = 0;i < ShotGun.fractions; i++) {
			...
			lastShot = Time.time;	
			//Play fire sound attached to a Weapon object
			GetComponent.<AudioSource>().clip = ShotGun.fireSound;
			GetComponent.<AudioSource>().Play();
			ShotGun.bulletsLeft--;
			//Send message to weapon animation script
			if(aimed && Aim.playAnimation){
	...
	function shotGunReload () {
		...
		//Play reload sound
		GetComponent.<AudioSource>().clip = ShotGun.reloadSound;
		GetComponent.<AudioSource>().Play();
		// Wait for reload time first - then add more bullets!
	...
	function knifeOneShot () {
		if (Time.time > knife.fireRate + lastShot){
			...
			firePointSetup();
			//Play fire sound attached to a Weapon object
			GetComponent.<AudioSource>().clip = knife.fireSound;
			GetComponent.<AudioSource>().Play();
			BroadcastMessage ("Fire", SendMessageOptions.DontRequireReceiver);
	...
	function Aiming(){
		...
		GetComponent.<Camera>().main.fieldOfView = Mathf.Lerp(GetComponent.<Camera>().main.fieldOfView, currentFov, Time.deltaTime/Aim.smoothTime);
	}
	/////////////////////////////////////////////////////////////////////////////////////////////////
	function cameraRecoilDo(){
		GetComponent.<Camera>().main.transform.localRotation = Quaternion.Slerp(GetComponent.<Camera>().main.transform.localRotation, camPos, Time.deltaTime * CameraRecoil.smooth); 
	}
